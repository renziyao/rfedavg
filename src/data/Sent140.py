...
def non_iid_percent(params):
    ...
    for k, v in dict(df[4].value_counts()).items():
        pick_sum += v
        usernames.append(k)
        if pick_sum > pick_need: break
    df_small = df.loc[df[4].isin(usernames)]
    df_small = (df_small.iloc[:pick_need, :]).sample(frac=1)
    df_train = (df_small.iloc[:int(pick_need * 0.8), :])
    df_test = df_small.iloc[int(pick_need * 0.8):, :]
    ...
    df_train_niid = df_train.iloc[int(s * df_train.shape[0]):, :].sort_values([4])
    p_train_iid = 0
    p_train_niid = 0
